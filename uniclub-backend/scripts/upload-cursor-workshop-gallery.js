...
// Image folder path
const imageFolder = path.join(__dirname, '../../public/cursor workshop');
...
async function uploadGalleryImages() {
  try {
    ...
    // Find the Cursor Workshop event by title
    console.log('🔍 Finding Cursor Workshop event...');
    const cursorEvent = await PastEvent.findOne({ 
      title: { $regex: /cursor/i } 
    });

    if (!cursorEvent) {
      console.error('❌ Cursor Workshop event not found in database');
      console.log('\n💡 Available events:');
    ...
    console.log(`✅ Found event: ${cursorEvent.title} (ID: ${cursorEvent._id})`);
    ...
    cursorEvent.gallery = gallery;
    await cursorEvent.save();

    console.log('✅ Gallery uploaded successfully!');
    console.log(`\n📸 Gallery images: ${gallery.length}`);
    console.log(`🔗 Event ID: ${cursorEvent._id}`);
    console.log(`📍 Access gallery at: /api/past-events/${cursorEvent._id}/gallery/:imageIndex`);
